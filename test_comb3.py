    ...
    for combination, lists in result.items():
        ...
        for key, value in result_with_stats.items():
            ...
            if curr_sum < min_sum:
                min_sum = curr_sum
                min_sum_key = key
            # Compare the current sum with the minimum sum found previously (`min_sum`)
            # and update `min_sum` and `min_sum_key` if the current sum is smaller.
...
def Draw_CDF(Diff,min_sum_key, venue_id, floor_id):
    sorted_data = np.sort(Diff)
    yvals = np.arange(len(sorted_data))/float(len(sorted_data)-1)
    plt.plot(sorted_data, yvals, label=min_sum_key)
    fig1 = plt.gcf()
    plt.title(f'venue_id: {venue_id}  floor_id: {floor_id}')
    plt.legend()
    plt.show()
    fig1.savefig(f'venue_id_{venue_id}  floor_id_{floor_id}'+'.png', dpi=100)
